...
```bash
cd D:\TotsFiles
uv run --no-sync --no-build --project Xylella_SPQR python Xylella_SPQR/baselines/run_grain_guard_baselines.py --smoke-test
uv run --no-sync --no-build --project Xylella_SPQR python Xylella_SPQR/baselines/run_grain_guard_baselines.py --workers 8
```
